...
var appendSuf = regexp.MustCompile("[0-9][a-zA-Z]+")
...
func ParseRule(s string) (r *rule, ok bool) {
	...
	if append := appendSuf.FindString(s); len(append) > 0 {
		r.appendSuf = appendSuf.FindString(s)[1:]
	} else {
		r.appendSuf = ""
	}
...
func (r *RuleTable) Stem(word string) string {
	...
	for cont {
		...
		for _, rule := range rules {
			...
			if !validStem(string(s) + rule.appendSuf) {
				continue
			}

			// All criteria passed, the word should be stemmed
			cont = rule.cont
			current = []rune(string(s) + rule.appendSuf)
...
func main() {
	...
	fmt.Println(rules.Stem("shitting"))
	fmt.Println(rules.Stem("играть"))
}
*/
